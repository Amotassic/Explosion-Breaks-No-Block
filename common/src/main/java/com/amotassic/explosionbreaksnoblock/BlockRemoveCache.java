...
public class BlockRemoveCache {
    ...
    public static boolean isBlockInTag(String tagName, Level level, Object... pos) {
        ...
        BlockState state = BuiltInRegistries.BLOCK.get(ResourceLocation.parse(getBlockId(level, pos))).defaultBlockState();
        return state.is(tagKey);
    }

    public static void onTick(Level level) {
        if (!level.isClientSide && level.dimension() == Level.OVERWORLD) blockRemoved.clear();
    }
}
